...
-- This script creates admin user and populates test data
...
INSERT INTO profiles (id, email, full_name, role, is_super_user, created_at, updated_at)
SELECT
  gen_random_uuid() as id,
  'admin@talk-to-my-lawyer.com' as email,
  'Admin User' as full_name,
  'admin' as role,
  true as is_super_user,
  NOW() as created_at,
  NOW() as updated_at
WHERE NOT EXISTS (
  SELECT 1 FROM profiles WHERE email = 'admin@talk-to-my-lawyer.com'
);

-- Ensure admin user has correct role and super_user status
UPDATE profiles
...
-- Create Test Subscribers with active subscriptions
...
-- Create active subscriptions for test subscribers
...
-- Create commission records for employees
...
-- Create test letters in various statuses
...
-- Create audit trail entries for letters
...
-- Update subscription employee_id for those with employee coupons
...
SELECT
  ...
  (SELECT COUNT(*)::text FROM letters WHERE status IN ('pending_review', 'under_review')) as pending_review,
  (SELECT COUNT(*)::text FROM letters) as total_letters,
  (SELECT COUNT(*)::text FROM profiles WHERE role = 'subscriber') as active_users,
  (SELECT SUM(price)::text FROM subscriptions WHERE status = 'active') as monthly_revenue;

-- Check letter distribution
SELECT
  'Letter Status Distribution' as category,
  status,
  COUNT(*)::text as count
FROM letters
GROUP BY status
